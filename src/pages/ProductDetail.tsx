  ...
  return (
    <div className="bg-gray-50 py-10">
      <div className="container mx-auto px-4">
        {/* Breadcrumb */}
        <div className="mb-6">
          <Link to="/products" className="inline-flex items-center text-lightblue hover:text-navy transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-16">
          ...
          <div>
            ...
            <div className="flex items-baseline mb-6">
              <span className="text-2xl font-bold text-navy">₹{product.price.toFixed(2)}</span>
              {product.oldPrice && (
        ...
        <div className="bg-white rounded-xl shadow-soft mb-16">
          <Tabs defaultValue="description" value={tab} onValueChange={setTab} className="w-full">
            <TabsList className="w-full border-b border-gray-200 rounded-none">
              <TabsTrigger value="description" className="flex-1 rounded-none rounded-tl-xl data-[state=active]:border-b-2 data-[state=active]:border-navy">
                Description
              </TabsTrigger>
              <TabsTrigger value="reviews" className="flex-1 rounded-none rounded-tr-xl data-[state=active]:border-b-2 data-[state=active]:border-navy">
                Reviews ({productReviews.length})
              </TabsTrigger>
            </TabsList>
            <TabsContent value="description" className="p-8">
              <div className="prose max-w-none">
                <h3 className="text-xl font-semibold text-navy mb-4">Product Description</h3>
                <p>{product.description}</p>
                
                {product.features && product.features.length > 0 && (
                  <>
                    <h4 className="text-lg font-semibold text-navy mt-6 mb-3">Key Features</h4>
                ...
                {product.specifications && (
                  <>
                    <h4 className="text-lg font-semibold text-navy mt-6 mb-3">Specifications</h4>
                    <table className="w-full border-collapse">
                      <tbody>
                        {Object.entries(product.specifications).map(([key, value]) => (
                          <tr key={key} className="border-b border-gray-200">
                            <td className="py-2 pr-4 font-medium w-1/3">{key}</td>
            ...
            <TabsContent value="reviews" className="p-8">
              <h3 className="text-xl font-semibold text-navy mb-4">Customer Reviews</h3>
              <div className="flex items-center gap-2 mb-6">
                <ReviewStars rating={averageRating} size="large" />
                <span className="text-2xl font-semibold text-navy">{averageRating.toFixed(1)}</span>
                <span className="text-gray-500">Based on {productReviews.length} {productReviews.length === 1 ? 'review' : 'reviews'}</span>
              </div>
